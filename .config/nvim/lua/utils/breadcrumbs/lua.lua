...
function M.get()
  ...
  local ok = pcall(vim.treesitter.get_parser, bufnr, "lua")
  ...
  while current do
    ...
    -- local x =
    if tp == "local_variable_declaration" then
      local names = current:field("names")
      if names and names[1] then
        table.insert(parts, 1, icons.localv .. text(names[1], bufnr))
      end
    end
