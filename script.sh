...
copy_configs() {
  ...
  rsync -a "$BASE/.config/" "$HOME/.config/" 2>/dev/null || true
  rsync -a "$BASE/.local/" "$HOME/.local/" 2>/dev/null || true
  rsync -a "$BASE/.themes/" "$HOME/" 2>/dev/null || true
  rsync -a "$BASE/.icons/" "$HOME/" 2>/dev/null || true
  rsync -a "$BASE/wallpapers/" "$HOME/Pictures/" 2>/dev/null || true
  ok "Configs copied"
}
